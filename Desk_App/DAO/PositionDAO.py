...
class PositionDAO:
    ...
    def get_all(self):
        self.cursor.execute("SELECT * FROM employee_position")
    ...
    def get_by_id(self, id):
        self.cursor.execute("SELECT * FROM employee_position WHERE id=%s", (id,))
        row = self.cursor.fetchone()
        return Position(**row) if row else None

    def add(self, position: Position):
        sql = "INSERT INTO employee_position (name,is_active) VALUES (%s,%s)"
    ...
    def update(self, position: Position):
        sql = "UPDATE employee_position SET name=%s,is_active=%s WHERE id=%s"
        self.cursor.execute(sql, (position.name, position.is_active, position.id))
        self.conn.commit()

    def delete(self, id):
        self.cursor.execute("DELETE FROM employee_position WHERE id=%s", (id,))
        self.conn.commit()
